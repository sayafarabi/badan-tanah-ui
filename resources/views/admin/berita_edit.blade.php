...
@section('content')
    ...
    <form action="{{ route('admin.berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <a href="{{ route('admin.berita.index') }}"
                            class="text-sm font-semibold text-red-600 hover:underline mt-1 inline-block">Kembali ke
                            Daftar</a>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if ($canEdit)
                    @if ($canEdit)
                        ...
                        {{-- SIMPAN DRAFT --}}
                        <button type="submit" form="submitBeritaForm"
                        ...
                        {{-- APPROVE --}}
                        <button type="submit" form="approveBeritaForm"
                        ...
                        {{-- PUBLISH --}}
                        <button type="submit" form="publishBeritaForm"
                            onclick="return confirm('Apakah Anda yakin ingin Publish berita ini?')"
                        ...
                        {{-- ARSIPKAN --}}
                        <button type="submit" form="unpublishBeritaForm"
                            onclick="return confirm('Apakah Anda yakin ingin Arsipkan berita ini?')"
                        ...
                        {{-- HAPUS --}}
                        <button type="submit" form="deleteBeritaForm"
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6">
            ...
            <span
                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-100 text-gray-600 text-xs font-medium">
            ...
            @if ($berita->tanggal_publikasi)
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 text-blue-600 text-xs font-medium">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="ringkasan" rows="3" placeholder="Masukkan ringkasan singkat berita"
                                class="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#006400]/30 focus:border-[#006400] transition"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="kategori" required
                                    ...
                                    {{ !$canEdit ? 'disabled' : '' }}>
                                    <option value="Berita"
                                        {{ old('kategori', $berita->kategori) == 'Berita' ? 'selected' : '' }}>Berita
                                    </option>
                                    <option value="Siaran Pers"
                                        {{ old('kategori', $berita->kategori) == 'Siaran Pers' ? 'selected' : '' }}>Siaran
                                        Pers</option>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div
                        class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center mb-6 hover:border-[#006400] transition">
                    ...
                    @if ($berita->gambar)
                        <div class="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-xl border border-gray-200">
                            <img src="{{ asset('storage/' . $berita->gambar) }}"
                                class="w-16 h-16 object-cover rounded-lg" alt="Gambar berita">
                            <div>
                                <p class="text-xs font-medium text-gray-700">Gambar saat ini</p>
                                <a href="{{ asset('storage/' . $berita->gambar) }}" target="_blank"
                                    class="text-xs text-blue-600 hover:underline">Lihat gambar</a>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="border border-gray-300 rounded-xl overflow-hidden">
                        <div class="bg-gray-50 border-b border-gray-200 p-2 flex flex-wrap gap-1 text-gray-600">
                            <button type="button" class="hover:bg-gray-200 p-1.5 rounded transition" title="Bold"><i
                                    class="fas fa-bold"></i></button>
                            <button type="button" class="hover:bg-gray-200 p-1.5 rounded transition" title="Italic"><i
                                    class="fas fa-italic"></i></button>
                            <button type="button" class="hover:bg-gray-200 p-1.5 rounded transition"
                                title="Underline"><i class="fas fa-underline"></i></button>
                            <span class="w-px h-6 bg-gray-300 mx-1"></span>
                            <button type="button" class="hover:bg-gray-200 p-1.5 rounded transition" title="List"><i
                                    class="fas fa-list-ul"></i></button>
                            <button type="button" class="hover:bg-gray-200 p-1.5 rounded transition" title="Link"><i
                                    class="fas fa-link"></i></button>
                            <button type="button" class="hover:bg-gray-200 p-1.5 rounded transition" title="Image"><i
                                    class="fas fa-image"></i></button>
                        </div>
                        <textarea name="konten" rows="10" required placeholder="Tulis konten berita di sini..."
                            class="w-full p-4 text-sm border-none focus:ring-0 resize-y" {{ !$canEdit ? 'disabled' : '' }}>{{ old('konten', $berita->konten) }}</textarea>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span
                                    class="absolute left-0 top-0 h-full flex items-center pl-4 text-gray-400 text-sm">/</span>
                        ...
                        <div>
                            ...
                            @if ($canEdit)
                                <select name="status" id="statusSelect"
                                    ...
                                    @if ($isAdmin || $isSuperAdmin)
                                        <option value="Draft"
                                            {{ old('status', $berita->status) == 'Draft' || $berita->status == 'Draft' ? 'selected' : '' }}>
                                            Draft</option>
                                        <option value="Menunggu Approval"
                                            {{ old('status', $berita->status) == 'Menunggu Approval' || $berita->status == 'Menunggu Approval' ? 'selected' : '' }}>
                                            Menunggu Approval</option>
                                        <option value="Terbit"
                                            {{ old('status', $berita->status) == 'Terbit' || $berita->status == 'Dipublikasikan' ? 'selected' : '' }}>
                                            Terbit</option>
                                    @elseif ($isEditor)
                                ...
                                <div class="mt-2 bg-blue-50 border border-blue-100 rounded-lg p-3">
                                    <div class="flex items-start gap-2">
                                        ...
                                        <p class="text-xs text-blue-700 leading-relaxed">
                                            Sebagai <strong>Editor</strong>, Anda hanya dapat mengubah ke
                                            <strong>Draft</strong>.
                                            Klik tombol <strong>"Submit untuk Approval"</strong> untuk mengirim ke
                                            Publisher.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="mt-2 bg-green-50 border border-green-100 rounded-lg p-3">
                                    <div class="flex items-start gap-2">
                                        ...
                                        <p class="text-xs text-green-700 leading-relaxed">
                                            Sebagai <strong>Publisher</strong>, Anda dapat mengubah status ke
                                            <strong>Draft</strong>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-4 text-sm">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-3">Timeline
                                Approval</p>
                            ...
                            @if (count($history) > 0)
                                <div
                                    class="relative pl-6 space-y-4 before:absolute before:left-1.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-200">
                                    @foreach ($history as $item)
                                        <div class="relative">
                                            <div
                                                class="absolute -left-5 top-1.5 w-3 h-3 rounded-full
                                            {{ $item['action'] == 'created'
                                                ? 'bg-gray-400'
                                                : ($item['action'] == 'submit'
                                                    ? 'bg-orange-500'
                                            ...
                                            <div
                                                ...
                                                <div>
                                                    ...
                                                    @if ($item['note'] ?? false)
                                                        <p class="text-[10px] text-gray-500 italic">{{ $item['note'] }}
                                                        </p>
                                                    @endif
                                                </div>
    ...
    @if ($canEdit && $isEditor && $isDraft)
        <form id="submitBeritaForm" action="{{ route('admin.berita.submit', $berita->id) }}" method="POST"
            class="hidden">
            @csrf
        </form>
    @endif

    @if ($canEdit && ($isPublisher || $isAdmin || $isSuperAdmin) && $isPending)
        <form id="approveBeritaForm" action="{{ route('admin.berita.approve', $berita->id) }}" method="POST"
            class="hidden">
            @csrf
        </form>
    @endif

    @if ($canEdit && ($isPublisher || $isAdmin || $isSuperAdmin) && ($isApproved || $isArchived))
        <form id="publishBeritaForm" action="{{ route('admin.berita.publish', $berita->id) }}" method="POST"
            class="hidden">
            @csrf
        </form>
    @endif

    @if ($canEdit && ($isPublisher || $isAdmin || $isSuperAdmin) && $isPublished)
        <form id="unpublishBeritaForm" action="{{ route('admin.berita.unpublish', $berita->id) }}" method="POST"
            class="hidden">
            @csrf
        </form>
    @endif
    ...
    @if ($canDelete)
        <form id="deleteBeritaForm" action="{{ route('admin.berita.destroy', $berita->id) }}" method="POST"
            class="hidden">
            @csrf
            @method('DELETE')
        </form>
